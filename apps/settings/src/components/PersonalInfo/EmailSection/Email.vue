...
export default {
	...
	methods: {
		...
		debounceEmailChange: debounce(function() {
			if ((this.$refs.email?.checkValidity() && this.containsNoWhitespace(this.email)) || this.email === '') {
				// $nextTick() is needed here to ensure that references to this.email further down the chain give the correct non-outdated value
